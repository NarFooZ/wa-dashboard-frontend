...
import { Plus, Search, RefreshCw, ChevronLeft, TrendingUp, TrendingDown, Ban, Archive, ShieldCheck } from 'lucide-react';
...
export default function Members() {
  ...
  const toggleBlock = async (m) => {
    try {
      ...
      toast.success(m.is_blocked ? 'تم رفع الحظر' : 'تم حظر العضو (موقوف مالياً)');
      load();
    } catch {
  ...
  const archive = async (m) => {
    if (!confirm(`أرشفة "${m.name || m.phone}"؟ سيتم الاحتفاظ بسجله المالي.`)) return;
    try {
      await api.post(`/members/${m.id}/archive`);
  ...
  const depositStatus = (m) => {
    if (!m.security_deposit_required) return null;
    const paid = m.security_deposit_paid >= m.security_deposit_required;
    return (
      <span className={`text-xs flex items-center gap-1 ${paid ? 'text-green-600' : 'text-orange-500'}`}>
        <ShieldCheck className="w-3 h-3" />
        {m.security_deposit_paid?.toFixed(0)}/{m.security_deposit_required?.toFixed(0)} أمان
      </span>
    );
  };
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-right px-4 py-3 font-semibold text-gray-600">الرصيد / الذمة</th>
              ...
              <tbody>
                {filtered.map(m => (
                  <tr key={m.id} className="table-row">
                    ...
                    <td className="px-4 py-3">
                      {m.is_blocked
                        ? <span className="badge-blocked">موقوف مالياً</span>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          ...
                          title={m.is_blocked ? 'رفع التعليق' : 'تعليق'}
