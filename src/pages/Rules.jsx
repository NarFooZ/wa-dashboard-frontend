...
import { Plus, Trash2, Edit2, Check, X, Zap, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function Rules() {
  ...
  const [form, setForm] = useState({ keyword: '', emoji: '', amount: '', description: '' });
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm({ keyword: '', emoji: '', amount: '', description: '' });
      setShowAdd(false);
      setEditId(null);
      load();
    } catch (err) {
  ...
  const startEdit = (rule) => {
    setEditId(rule.id);
    setForm({ keyword: rule.keyword, emoji: rule.emoji || '', amount: String(rule.amount), description: rule.description || '' });
    setShowAdd(true);
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowAdd(true); setEditId(null); setForm({ keyword: '', emoji: '', amount: '', description: '' }); }}
      ...
      <div className="card bg-green-50 border-green-200">
        ...
        <div className="mt-3 bg-white rounded-xl p-3 border border-green-100">
          <p className="text-xs text-gray-500 mb-1">مثال على رسالة التفعيل:</p>
          <p className="font-mono text-sm bg-wa-light px-3 py-2 rounded-lg inline-block">شراء 👍</p>
        </div>
      </div>
        ...
        <div className="card border-green-200">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input className="input" placeholder="مثال: شراء" value={form.keyword} onChange={e => setForm(p => ({ ...p, keyword: e.target.value }))} />
            </div>
            <div>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1 block">مبلغ الخصم (د.أ) *</label>
              <input type="number" min="0" step="0.01" className="input" placeholder="5.00" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} />
            </div>
            <div>
