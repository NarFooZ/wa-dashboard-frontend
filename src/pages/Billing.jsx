    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="card bg-green-50 border-green-200">
          ...
          <p className="text-2xl font-black text-green-700">+{total.credit.toFixed(2)} <span className="text-sm">د.أ</span></p>
        </div>
        <div className="card bg-red-50 border-red-200">
          <p className="text-xs font-semibold text-red-600">إجمالي الخصم</p>
          <p className="text-2xl font-black text-red-600">-{total.debit.toFixed(2)} <span className="text-sm">د.أ</span></p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex gap-2">
          {[['', 'الكل'], ['credit', 'شحن'], ['debit', 'خصم'], ['manual', 'يدوي']].map(([val, label]) => (
            <button
              key={val}
              ...
              {transactions.map(tx => (
                <div key={tx.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${tx.type === 'credit' ? 'bg-green-500' : 'bg-red-500'}`}>
                      {tx.type === 'credit' ? '↑' : '↓'}
                    </div>
                    <div>
